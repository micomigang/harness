...
TASK_PROMPTS = {
    ...
    "asset_manifest": (
        ...
        "Typed metadata uses a CLOSED required field set: character items include appearance and costume; scene items include key_set_elements; prop items include physical_description and used_in_scenes. These fields must be explicit and non-empty when evidence allows. Do not invent additional mandatory schema fields such as physical_tags, social_register, lighting_mood, spatial_function, architectural_style, material, dimensions_hint, or narrative_function unless the latest user instruction explicitly names them. These fields describe evidence-supported production traits and must not invent unsupported story facts. "
    ...
    "characters": (
        "Build the character bible from the approved screenplay AND asset_manifest. Obey each character manifest decision. For REUSE, preserve the referenced library asset identity; for VARIANT, preserve canonical identity and only change the bounded variant; for CREATE, design a new production-ready character automatically. "
        "VISUAL LOCALIZATION IS PART OF THIS STAGE. Separate source evidence from the final target-market production look. Treat manifest continuity_lock as source/narrative continuity constraints, not an instruction to copy culturally source-specific wardrobe literally. Preserve dramatic identity, age, role, class contrast, recognisable colour family/silhouette and story-critical accessories, while translating culturally specific clothes, grooming and styling into a plausible equivalent for workspace.settings.target_market. "
        "For France, do not leave an obviously source-culture rural costume merely because it appeared in the source. Example: a Chinese/source rural red floral padded coat should become a plausible French countryside quilted field jacket/coat in a related red or burgundy family with understated patterning, while preserving warmth, modest rural class signal and recognisability. Avoid random stereotypes. "
        "Return an items array. Each item needs id, manifest_id, name, appearance, wardrobe, performance, voice, used_in, continuity, canonical_key, reuse_decision (REUSE/VARIANT/CREATE), library_asset_id or null, source_continuity_lock, continuity_lock, production_continuity_lock, variant_key, reuse_decision_reason, source_visual_traits (array of source-observed visual anchors), localized_visual_design (target-market production design; object or concise structured text), visual_localization_notes (what was preserved vs culturally adapted), and generation_prompt_en (concise English visual-generation prompt for image/video providers; no dialogue). source_continuity_lock archives source-video visual continuity only. continuity_lock and production_continuity_lock MUST describe the localized target-market production continuity and MUST NOT copy source-culture styling merely to preserve history. Never make source_continuity_lock and continuity_lock identical when visual localization materially changes the design. "
        "The character bible prose may remain in the target story language, but generation_prompt_en must be English. Do not invent a new plot fact during visual localization."
    ),
    "scenes": (
        "Build scene/location specifications from the approved screenplay AND asset_manifest. Obey scene decisions: REUSE keeps the referenced library location unchanged; VARIANT preserves canonical layout/geography and only applies the requested state/time/lighting variation; CREATE automatically designs a reusable location. "
        "VISUAL LOCALIZATION IS PART OF THIS STAGE. Separate source visual evidence from target-market production design. Preserve spatial/narrative function, class contrast, geography and story-required anchors, but translate culturally source-specific architecture, decoration, furniture and domestic details into plausible workspace.settings.target_market equivalents. For France, favour coherent French bourgeois/haussmannien/maison-de-maitre or other script-supported French vocabulary and avoid Chinese/East-Asian decorative motifs unless the localized screenplay explicitly retains them. "
        "Return an items array. Each item needs id, manifest_id, name, location, layout, lighting, time_weather, used_in, continuity, canonical_key, reuse_decision, library_asset_id or null, source_continuity_lock, continuity_lock, production_continuity_lock, variant_key, reuse_decision_reason, source_visual_traits, localized_visual_design, visual_localization_notes, and generation_prompt_en. source_continuity_lock archives the source setting; continuity_lock/production_continuity_lock are the localized production authority and must not be auto-copied from source styling. generation_prompt_en is a concise English visual-provider prompt; the bible prose may remain in the story language. This is visual background design, never music."
    ),
    "props": (
        "Build only story-critical physical props from the approved screenplay AND asset_manifest. Obey prop decisions: REUSE keeps canonical identity; VARIANT preserves identity while applying a bounded state change; CREATE creates a new production prop when no series asset matches. "
        "VISUAL LOCALIZATION IS PART OF THIS STAGE. Preserve a prop's story function, recognisable geometry/state and continuity-critical traits, but translate culturally source-specific materials, ornament, tableware conventions, packaging or domestic styling into plausible workspace.settings.target_market equivalents when the localized story no longer lives in the source culture. For France, prefer plausible French/European domestic design and avoid Chinese/East-Asian motifs unless explicitly required by the localized screenplay. "
        "Each item needs id, manifest_id, name, description, material_scale, state, used_in, continuity, canonical_key, reuse_decision, library_asset_id or null, source_continuity_lock, continuity_lock, production_continuity_lock, variant_key, reuse_decision_reason, source_visual_traits, localized_visual_design, visual_localization_notes, and generation_prompt_en. source_continuity_lock archives source evidence; continuity_lock/production_continuity_lock are localized production continuity and must not inherit culturally source-specific styling by automatic union. generation_prompt_en must be English and purely visual."
    ),
